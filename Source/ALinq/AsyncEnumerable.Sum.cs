...
#pragma warning restore 1998
        ...
        public static Task<int> Sum<T>(this IAsyncEnumerable<T> enumerable, Func<T, int> selector)
        {
            return SumCore(enumerable.Select(selector), (current, next) => current + next);
        }

        public static async Task<int> Sum<T>(this IAsyncEnumerable<T> enumerable, Func<T, int?> selector)
        {
            return (await SumCore(enumerable.Select(selector), 0, (current, next) => next.HasValue ? current + next.Value : current).ConfigureAwait(false)).Value;
        }

        public static Task<long> Sum<T>(this IAsyncEnumerable<T> enumerable, Func<T, long> selector)
        {
            return SumCore(enumerable.Select(selector), (current, next) => current + next);
        }

        public static async Task<long> Sum<T>(this IAsyncEnumerable<T> enumerable, Func<T, long?> selector)
        {
            return (await SumCore(enumerable.Select(selector), 0L, (current, next) => next.HasValue ? current + next.Value : current).ConfigureAwait(false)).Value;
        }

        public static Task<float> Sum<T>(this IAsyncEnumerable<T> enumerable, Func<T, float> selector)
        {
            return SumCore(enumerable.Select(selector), (current, next) => current + next);
        }

        public static async Task<float> Sum<T>(this IAsyncEnumerable<T> enumerable, Func<T, float?> selector)
        {
            return (await SumCore(enumerable.Select(selector), 0.0f, (current, next) => next.HasValue ? current + next.Value : current).ConfigureAwait(false)).Value;
        }

        public static Task<double> Sum<T>(this IAsyncEnumerable<T> enumerable, Func<T, double> selector)
        {
            return SumCore(enumerable.Select(selector), (current, next) => current + next);
        }

        public static async Task<double> Sum<T>(this IAsyncEnumerable<T> enumerable, Func<T, double?> selector)
        {
            return (await SumCore(enumerable.Select(selector), 0.0, (current, next) => next.HasValue ? current + next.Value : current).ConfigureAwait(false)).Value;
        }

        public static Task<decimal> Sum<T>(this IAsyncEnumerable<T> enumerable, Func<T, decimal> selector)
        {
            return SumCore(enumerable.Select(selector), (current, next) => current + next);
        }

        public static async Task<decimal> Sum<T>(this IAsyncEnumerable<T> enumerable, Func<T, decimal?> selector)
        {
            return (await SumCore(enumerable.Select(selector), 0.0m, (current, next) => next.HasValue ? current + next.Value : current).ConfigureAwait(false)).Value;
        }
    }
